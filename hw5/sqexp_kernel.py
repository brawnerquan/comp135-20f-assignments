...
def calc_sqexp_kernel(x_QF, x_train_NF=None, length_scale=1.0):
    ...
    for i in range(Q):
        for  j in range(N):
            k_QN[i][j] = np.exp(-(np.sum(np.square(x_train_NF[j] - x_QF[i])) / np.square(length_scale)))
...
# print(k_11)
# # array([[1.]])
# print(calc_sqexp_kernel(x_one_11, x_one_11, length_scale=1.0))
# # array([[1.]])
# print(calc_sqexp_kernel(x_one_11, x_zero_11, length_scale=1.0))
# # array([[0.368]])
#
#
